...
class test_System(unittest.TestCase):
    ...
    def test_system_constructor_detail(self):
        ...
        #check attributes
        self.assertEqual(self.pot.constants[self.pot.nDim], sys.nDim, msg="Dimensionality was not the same for system and potential!")
    ...
    def test_append_state(self):
        ...
        sys = self.system_class(potential=self.pot, integrator=self.integ, conditions=[], temperature=temperature, position=position)
    ...
    def test_propergate(self):
        ...
        self.assertEqual(sys._currentTotPot, initialState.totPotEnergy, msg="The initialState  does not equal  the currentState after propergating in attribute: totPotEnergy!")
...
class test_perturbedSystem1D(test_System):
    ...
    def test_system_constructor(self):
        ...
        sys = system.perturbed_system.perturbedSystem(potential=pot, integrator=integ, position=position, temperature=temperature, lam=lam)
        curState = sys.getCurrentState()

        #check attributes
        self.assertEqual(pot.constants[pot.nDim], sys.nDim, msg="Dimensionality was not the same for system and potential!")
        self.assertEqual([], sys.conditions, msg="Conditions were not empty!")
        #print(curState)
        #check current state intialisation
        self.assertEqual(curState.position, expected_state.position, msg="The initialised Position is not correct!")
    ...
    def test_append_state(self):
        ...
        sys = system.perturbed_system.perturbedSystem(potential=pot, integrator=integ, position=position, temperature=temperature)
    ...
    def test_propergate(self):
        ...
        #check that middle step is not sames
        self.assertNotEqual(sys._currentPosition, initialState.position, msg="The initialState equals the currentState after propagating in attribute: Position!")
        self.assertEqual(sys._currentTemperature, initialState.temperature, msg="The initialState does not equal the currentState after propergating in attribute: temperature!")
