...
def animation_trajectory(
    ...
    if limits_coordinate_space is None:
        ...
        xtot_space = np.linspace(
            min(limits_coordinate_space), max(limits_coordinate_space) + 1, resolution_of_analytic_potential
        )
...
def animation_EDS_trajectory(
    ...
    if limits_coordinate_space is None:
        ...
        xtot_space = np.array(
            np.linspace(
                min(limits_coordinate_space), max(limits_coordinate_space) + 1, total_potential_resolution_points
            ),
            ndmin=1,
        )
    ...
    def run(data):
        ...
        if type(x) == type(x1data[-1]) == list and all(
            [xi == x1i for xi, x1i in zip(x, x1data[-1])]
        ):  # last step of traj
            curr_p.set_data([], [])
