...
def interactive_conveyor_belt(conveyorBelt=None, numsys: int = 8, nbins: int = 100, steps: int = 100):
    ...
    nReplicas_slider = ipywidgets.IntSlider(
        value=8, min=2, max=20, step=1, orientation="vertical", description="number of Replicas"
    )
...
class interactive_eds:
    ...
    def __init__(self, nstates=2, s=100, Eoff=None, figsize=[12, 6]):
        ...
        self.V_is = [
            pot.harmonicOscillatorPotential(x_shift=state * 4, k=10, unitless=True) for state in range(self.nstates)
        ]
        ...
        s_slider = ipywidgets.FloatSlider(
            value=100, min=0.1, max=101, step=1, orientation="horizontal", continous_update=True
        )
