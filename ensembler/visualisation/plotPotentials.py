...
def plot_1DPotential_V(
    ...
    if isinstance(energies, units.quantity):
        ene_unit = energies.units
        ene_magnitudes = energies.magnitude
    else:
        ene_unit = "kb T"
        ene_magnitudes = energies

    if isinstance(positions, units.quantity):
        pos_unit = positions.units
        pos_magnitudes = positions.magnitude
    else:
        pos_unit = "1"
        pos_magnitudes = positions

    if y_label is None:
        y_label = "$V\\ [" + str(ene_unit) + "]$"

    if x_label is None:
        x_label = "$r\\ [" + str(pos_unit) + "]$"
    ...
    # plot
    if color:
        ax.plot(positions, ene_magnitudes, c=color)
    else:
        ax.plot(positions, ene_magnitudes)

    ax.set_xlim(min(x_range), max(x_range)) if (x_range != None) else ax.set_xlim(
        min(pos_magnitudes), max(pos_magnitudes)
    )
    ax.set_ylim(min(y_range), max(y_range)) if (y_range != None) else ax.set_ylim(
        min(ene_magnitudes), max(ene_magnitudes)
    )
...
def plot_1DPotential_dhdpos(
    ...
    # generat Data
    dhdpos = potential.force(positions=positions)

    if isinstance(dhdpos, units.quantity):
        force_unit = dhdpos.units
        ene_magnitudes = dhdpos.magnitude
    else:
        ene_unit = "kb T"
        ene_magnitudes = dhdpos
    ...
    if y_label is None:
        y_label = "$\partial V / \partial r \\ [" + str(ene_unit) + "]$"
