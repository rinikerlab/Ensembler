...
from ensembler.util.units import quantity, unit_registry
...
def toggle_pint_matplotlib_support(f: callable) -> callable:
    """
       This function turns on the pint unit - Matplotlib support for each visualization function.
    ...
    @functools.wraps(f)
    def pint_mpl_supporter(*args, **kwargs) -> callable:
        unit_registry.setup_matplotlib()
        r = f(*args, **kwargs)
        unit_registry.setup_matplotlib(False)
        return r

    return pint_mpl_supporter
...
@toggle_pint_matplotlib_support
...
def plot_1DPotential_V(
    ...
    if isinstance(energies, quantity):
        y_label = "$V~[" + str(energies.units) + "]$"
    elif not y_label is None:
        y_label = "$V~[k_B T]$"

    if isinstance(positions, quantity):
        x_label = "$r~[" + str(positions.units) + "]$"
...
def plot_1DPotential_dhdpos(
    ...
    if isinstance(energies, quantity):
        y_label = "$\partial V / \partial r~[" + str(energies.units) + "]$"
    elif not y_label is None:
        y_label = "$\partial V / \partial r~[k_B T]$"
