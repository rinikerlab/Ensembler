...
from ensembler.util.ensemblerTypes import (
    samplerCls,
    conditionCls,
    potentialCls,
    Number,
...
class system(_baseClass):
    ...
    def set_current_state(
        ...
        self.currentState = self.state(
            self._currentPosition, self._currentTemperature, np.nan, np.nan, np.nan, np.nan, np.nan
        )
    ...
    def trajectory(self) -> pd.DataFrame:
        return pd.DataFrame(
            list(map(lambda x: x._asdict(), self._trajectory)), columns=list(self.state.__dict__["_fields"])
        )
    ...
    def __init__(
        ...
        self.initialise(
            withdraw_Traj=True, init_position=True, init_velocity=init_velocity, set_initial_position=start_position
        )
    ...
    def _init_velocities(self) -> NoReturn:
        ...
        if self.nStates > 1:
            ...
            self._currentVelocities = (
                [self._gen_rand_vel() for dim in range(self.nDimensions)]
                if (self.nDimensions > 1)
                else self._gen_rand_vel()
            )
    ...
    def _update_energies(self) -> NoReturn:
        ...
        self._currentTotE = (
            self._currentTotPot if (np.isnan(self._currentTotKin)) else np.add(self._currentTotKin, self._currentTotPot)
        )
    ...
    def simulate(
        ...
        if verbosity:
            iteration_queue = tqdm(
                range(steps), desc=_progress_bar_prefix + " Simulation: ", mininterval=1.0, leave=verbosity
            )
        else:
    ...
    def visualise(
        ...
        return simulation_analysis_plot(
            self,
            title=title,
            out_path=out_path,
            limits_coordinate_space=limits_coordinate_space,
            oneD_limits_potential_system_energy=oneD_limits_potential_system_energy,
            limits_force=limits_force,
            twoD_number_of_bins=twoD_number_of_bins,
