...
class torsionPotential(_potential1DCls):
    ...
    def __init__(self, wavePotentials=[wavePotential(), wavePotential(multiplicity=3)], radians=False):
        ...
        self.constants = {
            **{"wave_" + str(key): wave.V for key, wave in enumerate(wavePotentials)},
...
class envelopedPotential(_potential1DCls):
    ...
    V_functional = (
        -1
        / (beta * sis[0, 0])
        * sp.log(sp.Sum(sp.exp(-beta * sis[i, 0] * (Vis[i, 0] - Eoffis[i, 0])), (i, 0, nStates)))
    )
    ...
    def _initialize_functions(self):
        ...
        self.V_functional = (
            -1
            / (self.beta * self.sis[0, 0])
            * sp.log(sp.Sum(sp.exp(-self.beta * self.states[self.i, 0]), (self.i, 0, self.nStates - 1)))
        )
        self._update_functions()
    ...
    def _calculate_dvdpos_singlePos_overwrite(self, positions: (t.Iterable[float])) -> np.array:
        ...
        dVdpos_state = np.multiply(
            scaling, V_Is_dhdpos
        )  # np.array([(ene/V_R_part) * force for ene, force in zip(V_Is_ene, V_Is_dhdpos)])
...
class lambdaEDSPotential(envelopedPotential):
    ...
    V_functional = (
        -1
        / (beta * sis[0, 0])
        * sp.log(sp.Sum(lamis[i, 0] * sp.exp(-beta * sis[i, 0] * (Vis[i, 0] - Eoffis[i, 0])), (i, 0, nStates)))
    )
    ...
    def _initialize_functions(self):
        ...
        inner_log = sp.Sum(
            sp.Matrix(list(lamis.keys()))[self.i, 0] * sp.exp(-self.beta * self.states[self.i, 0]),
    ...
    def lam_i(self, lam: Union[Number, Iterable[Number]]):
        if isinstance(lam, Number) and self.constants[self.nStates] == 2:
            ...
            self._lam_i = np.array(
                [1 / self.constants[self.nStates] for x in range(self.constants[self.nStates])], ndmin=1
            )
            lamis = {"lam_" + str(i): self.lam_i[i] for i in range(self.constants[self.nStates])}
            self.constants.update({**lamis})
...
class flatwellPotential(_potential1DCls):
    ...
    def _calculate_energies(self, position: Union[Number, np.array]) -> Union[Number, np.array]:
        return np.squeeze(
            [
                self.y_min if (pos >= self.x_min and pos <= self.x_max) else self.y_max
                for pos in np.array(np.squeeze(position), ndmin=1)
            ]
        )
    ...
    def _calculate_dVdpos(self, positions: Union[Number, np.array]) -> Union[Number, np.array]:
        return np.squeeze(
            [
                np.inf if (pos == self.x_min or pos == self.x_max) else 0
                for pos in np.array(np.squeeze(positions), ndmin=1)
            ]
        )
...
class sumPotentials(_potential1DCls):
    ...
    def __init__(
        self,
        potentials: t.List[_potential1DCls] = (harmonicOscillatorPotential(), harmonicOscillatorPotential(x_shift=1)),
    ):
        """
        __init__
