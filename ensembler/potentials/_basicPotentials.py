...
class _potentialCls(_baseClass):
    ...
    __nStates: sp.Symbol = sp.symbols(
        "nStates"
    )  # this Attribute gives the amount of present states(interesting for free Energy calculus), please access via nStates
...
class _potentialNDCls(_potentialCls):
    ...
    def __init__(self, nDimensions: int = -1, nStates: int = 1, unitless: bool = False):
        """
            __init__
                This class constructs the potential class basic functions, initializes the functions if necessary and also does simplify and derivate the symbolic equations.
        Parameters
        ----------
        ...
        # Do unit management if required:
    ...
    def _update_functions(self):
        """
        This function is needed to simplify the symbolic equation on the fly and to calculate the position derivative.
        """

        self.V = self.V_functional.subs(
            self._constants_magnitude
        ).expand()  # expand does not work reliably with gaussians due to exp
        if not self._unitless:
            self.V_units = self.V_functional.subs(self._constants_units)

        self.dVdpos_functional = sp.diff(
            self.V_functional, self.position
        )  # not always working! - depends on the complexity of the functional - limits by sympy!
...
class _potential1DCls(_potentialNDCls):
    ...
    def force(self, positions: Union[Iterable[Number], Number]) -> Union[Iterable[Number], Number]:
        """
            force
                calculates the potential forces/gradients of the given position/s using the derivative of potential function with the position.
    ...
    def visualize(
        ...
        positions: np.array,
        out_path: str = None,
        x_range: np.array = None,
        y_range: np.array = None,
        title: str = None,
    ) -> Figure:
        """
            TODO: Write
        ...
        positions : np.array, optional
            _description_, by default positions
        out_path : str, optional
            _description_, by default None
        x_range : np.array, optional
            _description_, by default None
        y_range : np.array, optional
            _description_, by default None
        title : str, optional
            _description_, by default None
